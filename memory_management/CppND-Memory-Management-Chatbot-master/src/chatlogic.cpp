...
ChatLogic::~ChatLogic()
{
    ...
    // delete all nodes - not needed as unique ptrs are deleted automatically
    // vector has smart pointer implementation so will also be deleted automatically
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            edge->SetChildNode((*childNode).get()); // dereference iterator then get node
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get()); // child just has reference
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // parent owns
                        }
